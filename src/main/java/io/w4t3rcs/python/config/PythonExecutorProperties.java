...
public record PythonExecutorProperties(@DefaultValue("local") Type type, LocalProperties local, RestProperties rest, GrpcProperties grpc) {
    ...
    public record LocalProperties(@DefaultValue("python") String startCommand) {
    }
    ...
    public record RestProperties(@DefaultValue("http://0.0.0.0") String host,
                                 ...
                                 String username,
                                 String password,
                                 @DefaultValue("${spring.python.executor.rest.host}:${spring.python.executor.rest.port}/script?username=${spring.python.executor.rest.username}&password=${spring.python.executor.rest.password}") String uri) {
    }
